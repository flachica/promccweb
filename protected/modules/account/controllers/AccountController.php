...
class AccountController extends Controller
{
	...
	public function actionRegister()
	{
		...
		if(isset($_POST['Account']))
		{
            ...
            // Send verification mail
				...
				// Redirect
				Yii::app()->user->setFlash('notice','Para completar el registro revise su correo');
	...
	public function actionCompleteRegister($account_id, $code)
	{
		...
		if($verification)
		{
			if($verification->validateCode($code))
			{
				...
				Yii::app()->user->setFlash('success','El registro ha sido completado');
			}
			else
			{
				Yii::app()->user->setFlash('error','El registro no ha podido ser completado');
			}
		}
		else
		{
			Yii::app()->user->setFlash('notice','El registro ya estaba completado');
		}
		...
		if(isset($_POST['Account']))
		{
			...
			if($model->validate())
			{
				...
				if($account->validatePassword($model->password))
				{
					...
					// Redirect
					Yii::app()->user->setFlash('notice','Para completar el cambio de cuenta de correo revise su email');
					$this->redirect(array('account'));
				}
				else
				{
					$model->addError('password','Contraseña incorrecta.');
				}
			}
		}
	...
	public function actionCompleteChangeEmail($account_id, $code)
	{
		...
		if($verification)
		{
			if($verification->validateCode($code))
			{
				...
				Yii::app()->user->setFlash('success','El cambio de correo ha sido completado');
			}
			else
			{
				Yii::app()->user->setFlash('error','El cambio de correo no ha sido completado');
			}
		}
		else
		{
			Yii::app()->user->setFlash('notice','El cambio de correo ya estaba completado');
		}
	...
	public function actionChangePassword()
	{
		...
		if(isset($_POST['Account']))
		{
			...
			if($model->validate())
			{
				...
				{
					$model->addError('oldPassword','Contraseña antigua incorrecta.');
				}
			}
		}
	...
	public function actionResetPassword()
	{
		...
		if(isset($_POST['Account']))
		{
			...
			if($model->validate())
			{
				...
				// Redirect
				Yii::app()->user->setFlash('notice','Para completar el reseteo de la contraseña revise su correo');
				$this->redirect(Yii::app()->homeUrl);
			}
		}
	...
	public function actionCompleteResetPassword($account_id, $code)
	{
		...
		if($verification)
		{
			if($verification->validateCode($code))
			{
				...
				if(isset($_POST['Account']))
				{
					...
					if($model->validate())
					{
						...
						Yii::app()->user->setFlash('success','El reseteo de su contraseña ha sido completado');
						$this->redirect(Yii::app()->user->returnUrl);
					}
				}
			...
			{
				Yii::app()->user->setFlash('error','El reseteo de su contraseña no ha sido completado');
			}
		}
		else
		{
			Yii::app()->user->setFlash('notice','El reseteo de su contraseña ya estaba completado');
		}
	...
	public function actionDesactivate()
	{
		...
		if(isset($_POST['Account']))
		{
			...
			if($model->validate())
			{
				...
				{
					$model->addError('password','Contraseña antigua incorrecta.');
				}
			}
		}
